            ...
            <Reveal key={item.id} delay={index * 90} className="h-full">
              <article className="interactive-card card flex h-full flex-col p-5 sm:p-6">
                <div className="flex items-start justify-between gap-4">
                  <div className="text-xs font-semibold text-green-300">{item.label}</div>
                ...
                <p className="mt-3 text-[0.93rem] leading-7 text-zinc-400">{item.cardDescription}</p>

                <div className="mt-5">
                  <p className="text-xs font-semibold uppercase tracking-[0.13em] text-zinc-500">My contribution</p>
                  <p className="mt-2 text-[0.93rem] leading-7 text-zinc-300">{item.cardContribution}</p>
                </div>
